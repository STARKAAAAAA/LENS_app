...
| 版本 | 更新 |
|------|------|
| v1.6.1 | 自定义标题栏、快捷键提示面板、手柄全功能导航（方向键+摇杆+按钮映射+自动切换输入源）、卡片悬浮浮游微动+光晕动画、侧边栏手柄操作（两步展开/文件夹切换/删除）、Xbox风格按钮提示图标（手柄/键盘按设备切换）、Hero区域滚动手势、网格边界跨行回绕修复、画廊左右同行走位算法、光晕独立DOM元素自适应形状、Aldrich字体离线打包、CSS/JS模块化拆分（12+10文件）、Rust端错误日志 |
